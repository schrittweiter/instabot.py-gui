...
class InstaBot:
                            ...
                            if like != 0:
                                if like.status_code == 200:
                                    ...
                                    insert_media(self,
                                                 ...
                                                 status="200")
                                    eel.bot_image(datetime.datetime.now().strftime("%d.%m.%Y %H:%M"),self.get_instagram_url_from_media_id(self.media_by_tag[i]['node']['id']))                                    
                                    self.write_log(log_string)
    ...
    def write_log(self, log_text):
        ...
        if self.log_mod == 0:
            ...
            try:
                now_time = datetime.datetime.now()
                eel.bot_log(now_time.strftime("%d.%m.%Y - %H:%M")  + " " + log_text)
                eel.bot_console(now_time.strftime("%d.%m.%Y - %H:%M")  + " " + log_text)
            except UnicodeEncodeError:
                eel.bot_log("Your text has unicode problem!")
                eel.bot_console("Your text has unicode problem!")